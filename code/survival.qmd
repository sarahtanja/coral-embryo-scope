...
mutate(treatment = factor(treatment, levels = c("control", "low", "mid", "high"), ordered = TRUE),
          hpf = factor(hpf, levels = c("4", "9", "14", ordered = TRUE)))

head(tidy_vials %>% select(sample_id, treatment, hpf, n_viable, n_embryos))
```
...
```
```{r}
# Get the four PuRd colors
leachcolors <- brewer.pal(9, "PuRd")[c(3, 7, 8, 9)]
leachcolors
...
```{r}
leachate_colors <- c("#D4B9DA", "#CE1256", "#980043", "#67001F","black")
stage_colors <- c("#FEE6CE", "#FDAE6B", "#F16913", "#A63603")
```

# Boxplot
```{r}
box <- ggplot(tidy_vials, aes(x = hpf, y = n_viable, fill = treatment)) +
  geom_boxplot(alpha = 0.9, outlier.shape = ".") +
...
```{r}
# Create a data frame for the single point with error bars
...
box2 <- box +
  geom_pointrange(data = single_point, aes(x = hpf, ymin = ymin, ymax = ymax), color = "black")+
