...
The multivariate results tell us that treatment as a whole doesn't have
a significant effect on stage composition (p=0.999), but developmental
stage (hpf) has a huge effect (p=0.001), and the interaction between
treatment and hpf is also significant (p=0.001). This means that the
effect of leachate on stage composition depends on developmental stage,
even though there is no overall treatment effect. The treatment effects
are stage-specific, subtle, or even opposing across stages, which is why
we see a significant interaction without a main effect of treatment.

The univariate results tell us that the prawnchip stage interaction term
P = 0.036 is driving the significant interaction, with a significant
effect of treatment at 14 hpf. The other stages show borderline or
non-significant effects, suggesting that the overall multivariate
interaction is being driven mostly by changes in the prawnchip stage at
14 hpf.

Prawnchip counts differ by treatment at 14 hpf The relationship between
treatment and prawnchip counts changes across time This includes 14 hpf,
but depends on the whole trajectory from 4 to 14 hpf Treatment is
slightly altering how embryos move through the prawnchip stage... either
lingering longer in prawnchip, or exiting prawnchip faster.

> Univariate tests suggest that this interaction is primarily driven by
> differences in the prawnchip stage, indicating that treatment may
> alter the timing of progression through this intermediate
> developmental stage.

or

> This pattern is consistent with subtle shifts in developmental timing,
> where embryos may transition into or out of the prawnchip stage at
> different rates under leachate exposure.
:::
...
how much that estimated difference would vary if the experiment were
repeated

That is what the bootstrap CI is doing.

A prediction grid is a table of the combinations we want predictions
for. Because our model used an offset, we need to give the grid an N_obs
value to get predictions on the scale of proportions rather than counts.
We set N_obs = 1 to get predictions of proportions, and this allows us
to remove differences due to vial size and focus on composition-like
fitted counts. I think of it as counts normalized to a common total of
1, which is equivalent to modeling proportions.
...
Here we ask the model, "For each treatment and time combination, what is
the expected count of embryos in each stage, assuming a total of 1
embryo (N_obs = 1)?" This gives us predicted proportions for each stage
under each treatment and time condition, which we can then use to
calculate differences from control and visualize with confidence
intervals.
...
This returns predicted counts for: n_egg n_cleavage n_morula n_prawnchip
n_earlygastrula

These are model-based expected stage counts for every treatment and
timepoint.
...
Right now you have one model fit, one set of predicted differences. But
if you repeated the experiment with slightly different vials, you would
not get exactly the same answer. The bootstrap mimics that. It asks: “If
I repeatedly re-sampled my vials and refit the model, how much would
these treatment-control differences move around?” That gives uncertainty
around each point estimate. Sample the vials with replacement from your
original dataset. This creates a new pseudo-dataset the same size as the
original, but with some vials repeated and some left out. Conceptually,
this is one “alternate version” of your experiment.
...
one_boot(...) = one resample, one refit, one set of treatment-control
differences boot_preds = all bootstrap replicates stacked together
...
Each horizontal line represents: “How much would this treatment −
control difference change if we repeated the experiment 500 times?”

forest plot notes: ✅ built from predicted differences ✅ uncertainty
from bootstrap (not model SE) ✅ intervals often cross 0 ✅ mall effect
sizes, ow and miid levels of leachate speeding up embryonic development
very slightly?
...
What the bootstrapped data shows.. maybe speeding up development very
slightly?
