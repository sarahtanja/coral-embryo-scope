...
> You first need to convert your count data from wide format (with separate columns for each stage) to long format with three columns:
- treatment (factor for treatment groups)
- stage (one of the 5 embryo stages)
- count (number of embryos in that stage)
This long format allows ggplot2 to map stage to the fill aesthetic for stacking.
...
> At 4 hpf, the majority of embryos are indeed in the egg or early cleavage stages, and very few or none are in advanced stages like prawnchip or early gastrula. 
As time progresses to 9 and then 14 hpf, the embryo counts in the earlier stages (egg, cleavage) decrease, reflecting development progression, while counts in the later stages (prawnchip, early gastrula) increase correspondingly.
...
## Negative binomial per stage
```{r}
# Fit a negative binomial model per stage
glm_list <- long_timing_counts %>% 
  group_by(hpf_factor) %>%
  do(model = glm.nb(n ~ treatment, data = .))
```

```{r}
ltc_4 <- long_timing_counts %>% 
  filter(hpf == 4)

nb4 = glm.nb(n ~ treatment, data = ltc4)
```
...
# 
```{r}
ggplot(data=tidy_vials, aes(x = reorder(sample_id, hpf), y = n_embryos))+
  geom_col()+
  theme_minimal()+
  theme(
...
Multinomial assumes that variance = mean × (1 – mean) × n. In practice,
...
Roughly between -2 and +2 for each stage → model fit looks fine (no
extreme outliers).
...
Intercept = -1.36 → under control, average log(cleavage/egg) is negative
...
→ No significant treatment effect on cleavage proportions relative to
egg.

For morula vs egg:

Intercept = -1.19 → under control, egg is more likely than morula
(log-ratio negative).
...
Intercept = 1.22 → describes how concentrated the distributions are (a
measure of dispersion around the mean proportions).

Significant (p \< 0.001), which is typical — but you usually don’t
...
> (estimate = –1.36, SE = 0.36, z = –3.79, p \< 0.001) and morula
> (estimate = –1.19, SE = 0.35, z = –3.40, p \< 0.001), indicating that
...
statistic (lower = better). - Convergence happened in 4 steps — ✅ good.
...
mid treatments. stageegg:treatmenthigh = strong positive interaction →
...
groups. - This value is on the logit scale — not typically of primary
...
inflation — so most values are in (0,1), and zero-inflation didn’t
dominate. - Not significant → zeros may not be strongly patterned.
...
especially in the egg stage under high and mid treatments → more embryos
...
✅ Safe to ignore as long as you’re aware — it’s essentially a built-in
pseudocount.
:::
...
> confirmed that the odds of being in egg (estimate = –2.75, SE = 0.32,
> p \< 0.001), cleavage (estimate = –2.98, SE = 0.34, p \< 0.001), or
> morula (estimate = –2.37, SE = 0.30, p \< 0.001) were significantly
...
> chip (estimate = –3.08, SE = 0.31, z = –10.04, p \< 0.001), morula
> (estimate = –3.32, SE = 0.32, z = –10.34, p \< 0.001), or egg
> (estimate = –3.46, SE = 0.33, z = –10.53, p \< 0.001) were all
