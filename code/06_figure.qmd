...
## Embryo counts for each stage category 
```{r}
...
```{r}
stage_legend_plot <- ggplot(stage_legend_df, aes(x = 0, y = stage)) +
  geom_image(aes(image = img), size = 0.14) +
...
## Embryo counts for each status category
```{r}
...
status_legend_df <- tibble::tibble(
  status = factor(c("malformed", "uncertain", "typical"),
                    levels = c("malformed", "uncertain", "typical"), 
                    ordered = TRUE),
  label = c("Typical", "Uncertain/Torn", "Malformed"),
  img = c('../legend/typical.png', 
...
status_legend_plot <- ggplot(status_legend_df, aes(x = 0, y = status)) +
  geom_image(aes(image = img), size = 0.19) +
  geom_text(aes(label = label), 
            hjust = 0, nudge_x = 0.0014, 
            color = "grey40", size = 8) +
...
## Legend patch
```{r}
B_panel <- timing + stage_legend_plot + plot_layout(widths = c(4, 1))
C_panel <- abnormality + status_legend_plot + plot_layout(widths = c(4, 1))
```
...
# Save figure
```{r}
ggsave("../figs/figure_survival_timing_morphology.png", width = 10, height = 12, dpi = 900)
```
