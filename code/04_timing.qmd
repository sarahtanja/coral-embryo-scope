...
# 2. get fitted values from the model
pred_mat <- predict(mod_all, newdata = pred_grid, type = "response")

# 3. turn predictions into a tidy dataframe
pred_df <- pred_grid %>%
  bind_cols(as.data.frame(pred_mat)) %>%
  pivot_longer(
    cols = starts_with("n_"),
    names_to = "stage",
    values_to = "fitted_count"
...
# 4. extract control fitted values at each hpf and stage
control_df <- pred_df %>%
  filter(treatment == "control") %>%
  select(hpf, stage, control_fitted = fitted_count)

# 5. subtract control from every treatment within hpf and stage
diff_df <- pred_df %>%
...
```
```{r}
pred_df_prop <- pred_grid %>%
  bind_cols(as.data.frame(pred_mat)) %>%
  mutate(total_fitted = n_egg + n_cleavage + n_morula + n_prawnchip + n_earlygastrula) %>%
  pivot_longer(
    cols = starts_with("n_"),
    names_to = "stage",
    values_to = "fitted_count"
  ) %>%
  mutate(fitted_prop = fitted_count / total_fitted)

control_prop_df <- pred_df_prop %>%
  filter(treatment == "control") %>%
  select(hpf, stage, control_prop = fitted_prop)

diff_prop_df <- pred_df_prop %>%
  left_join(control_prop_df, by = c("hpf", "stage")) %>%
  mutate(diff_from_control = fitted_prop - control_prop) %>%
  filter(treatment != "control")


ggplot(diff_prop_df, aes(x = hpf, y = diff_from_control, color = treatment, group = treatment)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey50") +
  geom_line() +
  geom_point(size = 2) +
  facet_wrap(~ stage) +
  labs(
    title = "Treatment minus control differences in fitted stage proportions",
    subtitle = "Positive values indicate a higher stage proportion than control",
    x = "Hours post-fertilization",
    y = "Difference from control (fitted proportion)",
    color = "Treatment"
  ) +
  theme_bw()
```
